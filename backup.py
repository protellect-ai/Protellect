...
def render_microbiome_page():
        ...
        if st.button("Assess Batch Quality", key="mic_batch_run") and batch_input:
            ...
            for line in lines_b:
                ...
                st.markdown(
                    ...
                    f"<span style='color:{colcon_v}</span>"
    ...
    # ── Tab 2: Taxonomy Intelligence ─────────────────────────────────────────
        ...
        displaya = {k: v for k, v in MICROBE_TAXONOMY_KB.items() 
...
dr();
    ...
    # ── Tab 3: Host-Microbe Interactions ─────────────────────────────────────
        ...
        interactions = [
            ...
            ("PSA (Bacteroides fragil)", "TLR2", "#22c55e", "Polysaccharide A from B. fragilis → TLR2 → Treg induction → IL-10 production → tolerance. Protective in gnotobiotic models. Capsule-specific and symbiont-specific."),
            ...
            ("Indole/Serotonin", "AhR/5-HT3/4", "#a855f7",  indole/IPA (Bacteroides/Lactobacillus) → AhR → gut barrier + IL-22. Enterochromaffin cells: tryptophan → 5-HT → bowel motility. Serotonin pathway alteration in IBS/depression."),
        ]
    ...
    # ──: BGC Analysis ──────────────────────────────────────────────────
        ...
        bgc_typ           ("Type I PKS", "#ff8c42", "Modular polyketide synthase. Multi-domain assembly line: KS-AT-DH-KR-ACP. Produces: erythromycin, rapamycin, epothilone. Detect: antiSMASH pks1 rule. Signature: AT acyltransferase domain substrate specificity."),
            ...
            PP", "#22c55e", "Ribosomally synthesised and post-translationally modified peptide. Includes: lanthipeptides (nisin), thiopeptides (thiostrepton), bacteriocins, lasso peptides. Detect by: core peptide + modification enzymes."),
...
def render_pharma_workspace():
    ...
    iv style='color:#003a50;font-size:.82rem;line-height:1.6;'>
    ...
    with ph1:
        ...
        steps_filamin = [
            ("Day 1", "Co-immunoprecipitate FLNA from GPCR-expressi cells (HEK293 or native tissue). Use anti-FLNA Ig21 antibody (Millipore clone 3/F9)."),
            ...
            ("Interpretation", "Ser2152 pphorylation = proximal GPCR activation readout (more proximal than β-arrestin or cAMP). Works for ~300 Class A GPCRs carrying H8 motif."),
        ]
        ...
        st.makdown("<a href='https://pubmed.ncbi.nlm.nih.gov/26124276/' target='_blank' style='color:#00d4ff;font-size:.7rem;'>PMID:26124276 ↗</a>", unsafe_allow_html=True)
    ...
    with ph2:
        ...
        admet = [
         ...
         omal stability", "t1/2 >60min (human LM)", "Metabolic stability"),
            ("Ames test", "Negative", "Genotoxicity — IND-enabling requirement"),
        ]
        for rule, criteria, note in admet:
            st.markdown(f"<div style='display:flex;gap:6px;padding:3px 0;border-bottom:1px solid #050e18;'><span style='color:#00d4ff;font-size:.67rem;min-width:120px;font-weight:600;'>{rule}</span><span style='color:#3a6080;font-size:.67rem;'>{criteria} <span style='color:#1e4060;'>({note})</span></span></v>", unsafe_allow_html=True)
    ...
    timeline = [
        ...
        ("Lead Optimisation", "Ye6f1", "Medicinal chemistry: potency↑, selectivity↑, ADMET↑. Ro5 compliance. Selectivity panel (KINOMEscan/GPCRome). hERG safety. Rat PK (F%, t1/2, Cmax)."),
        ...
        ("Phase I (FIH safety)", "Year 7–9", "#ff8c42", "First-in-human. Dose escalation (DLT). PK/PD. MTD. ~20–80 healthy volunteers or patients. Durears."),
        ...
        ("Regulatory review → Approval", "Year 15–17", "#22c55e", "NDA/BLA/MAA filing. Priority review if rare disease/breakthrough. FDA review 10–12 months. -marketing commitments."),
    ]
...
def render_molbio_workspace():
    ...
    with mb1:
        ...
        kinase_data = [
            ("PKA (PRKACA/", "cAMP-activated. Consensus: [RK]-x-x-[ST]. Key substrates: CREB-Ser133, FLNA-Ser2152, RYR2-Ser2808, CFTR-Ser768. Inhibitor: H89 (IC50 48nM), PKI peptide (for specificity control)."),
            ...
            ("GSK3β", "Constitutively active unless phospho-inhibited (Akt→Ser9). Substrates: tau-Ser396/Ser404, glycogen sytenin. Lithium inhibits at mM. Inhibitor: SB216763 (IC50 34nM)."),
            ...
            ("ERK1/2 (MAPK)", "RAS→RAF→MEK→ERK cascade. Substrates: RSK, Elk-1, p90r202/Tyr204 dual phosphorylation activates. Inhibitor: SCH772984 (ERK selective)."),
    ...
    with mb2:
        ...
        sb_steps = [
            ...
            ("2 ·et druggability", "Identify pockets (volume, hydrophobicity, druggability score >0.5). Download AlphaFold PDB → run fpocket locally or fpocket-web. Classify: cryptic, allosteric, orthosteric."),
            ...
            ("4 · HDX-MS for dynamics", "Maps regions of conformational flexibility. Deuterium uptake = accessible/mic. Ligand-protected = binding site or allosteric change. Resolution: peptide level (~10 aa)."),
...
for k,v0 in {"pdata":None,"cv":None,"pdb":"","papers":[],"scored":[],"gene":"","uid":"",
             ...
             "partner_cv":None,"partner_gi":None,"disease_search":"","disease_proteins":[],"csv_triage_ase,"show_tutorial":True,"gnomad":{},"string":[],"trials":[],"drugs":[],"abstracts":[],"org":{},"ai_result":{},"ot":{},"am":{},"isoforms":[],"hotspots":[],"patients":{},"excel_bytes":None,
...
if _cur_domain != _last_domain and _cur_domain is not None:
    ...
    for _clr_key in ["pdata","cv","pdb","scored","gene","uid","assay","last",
        ...
        st.session_state[_clr_key] = [] if isinstance(st.session_state.get(_clr_key), list) else ({} if isitance(st.session_state.get(_clr_key), dict) else (None if _clr_key not in ["pdb","gene","uid","assay","last"] else ""))
...
def render_rare_disease_workspace():
    ...
    <div class='rd-header'>
      ...
      <div>
        ...
        <div style='cor:#3a1060;font-size:.78rem;'>VUS Triage · HPO → Candidate Genes · ACMG Classification · Inheritance · Functional Validation</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
RIORITISATION ENGINE ────────────────────────────────────────────
    if rd_mode == "🧬 VUS Prioritisation Engine":
        ...
        with vc1:
            ...
            vus_tyst.selectbox("Variant type", ["Missense","Nonsense (stop-gain)","Frameshift","Splice site","In-frame indel","Synonymous","UTR","Deep intronic","CNV/SV"], key="vus_type")
        with vc2:
            ...
            vus_clinvar_stars = st.selectbox("ClinVar review status", ["Not in ClinVar","0 stars (cong/no criteria)","1 star (single submitter)","2 stars (multiple submitters)","3 stars (expert panel)","4 stars (practice guideline)"], key="vus_cv_stars")
        ...
        if st.button("⚡ Prioritise this variant", type="primary", key="vus_run", use_container_width=True):
            ...
            if vus_type == "Missense":
                ...
                else: scores["AlphaMisse f"AM={vus_am:.2f} — likely benign", "#ff2d55", "BP4 applicable")
            else:
            ...
            elif vus_type in [nse","In-frame indel"]: scores["Variant type"] = (1, f"{vus_type} — functional data needed", "#ffd60a", "PS3 required for strong evidence")
        ...
        cv_s = star_score.get(vus_clinvar_stars, 0)
            ...
            else: scores["ClinVar"] = (0, "Not in ClinVar or conflicting", "#3a6080", "No ClinVar support �eed de novo evidence")
            # 5. Structural (pLDDT)
            ...
            else: scores["AlphaFold pLDDT"] = (0, f"pLDDT={vus_pldt} — disordered", "#ff8c42", "IDR — AlphaMi less informative")
            # 6. Gene constraint
            ...
            else: scores["Gene constraint (pLI)"] = (0, f"pLI={vus_pli:.2f} — tolerates LoF", "#ff8c42", "LoF variants lessy disease-causing")
            ...
            st.markdown(f"<div style='background:{tier_clr}0d;border:2px solid {tier_clr}44;boius:12px;padding:1rem 1.5rem;margin:.8rem 0;text-align:center;'>"
            ...
            steps_map = {
                ...
                "Tier 2": [("First","AlphaFold structural analysis — map variant position against domain architecture and binding Week 1","Minigene splice assay if near splice site (within 20nt of exon boundary)"),("Week 2","Patient cell-based assay — does protein localise correctly? Is stability normal? (Western + IF)"),("Week 3","Computational co-segregation check — request pedigree data from referring clinician"),],
                "Tier 3": [("First","Check if gene-disease relationship is ClinGen-validated (Definitive/Strong/Moderate only)"),("Week 1","RNA-seq from patient blood — look for aberrant splicing or ASE before wetnvestment"),("Consider","Functional assay only if multiple families with same variant — insufficient single-family evidence"),],
            ...
            if vus_gene and st.button(f"→ Full protn analysis: {vus_gene}", key="vus_analyse_btn", type="primary", use_container_width=True):
                st.session_state["_trigger_search"] = vus_gene; st.rerun()

    # ── HPO → CANDIDATE GENES ────────────────────────────────────────────────
    elif rd_mode == "🔬 HPO → Candidate Genes":
        sh("🔬", "Phenotype → Candidate Gene Finder")
        st.markdown("<div style='color:#3a6080;font-sgin-bottom:.6rem;'>Select patient phenotype categories → ranked candidate genes. Start with the most specific phenotype. Multiple selections = AND logic (gene must explain all features).</div>", unsafe_allow_html=True)
