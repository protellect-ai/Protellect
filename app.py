...
LOGO_SVG_RAW = """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" width="200" height="200">
  ...
  <!-- Outer ring -->
  <circle cx="100" cy="100" r="90" fill="none" stroke="#38bdf8" stroke-width="0.6" opacity="0.15"/>
  <circle cx="100" cy="100" r="75" fill="none" stroke="#38bdf8" stroke-width="0.4" opacity="0.1"/>
  ...
  <g filter="url(#softglow)" opacity="0.9">
    <line x1="82" y1="30" x2="112" y2="30" stroke="#38bdf8" stroke-width="2" stroke-linecap="round"/>
    <line x1="74" y1="48" x2="118" y2="48" stroke="#38bdf8" stroke-width="1.8" stroke-linecap="round" opacity="0.7"/>
    <line x1="70" y1="66" x2="122" y2="66" stroke="#38bdf8" stroke-width="2.2" stroke-linecap="round"/>
    <line x1="74" y1="84" x2="118" y2="84" stroke="#38bdf8" stroke-width="1.8" stroke-linecap="round" opacity="0.7"/>
    <line x1="84" y1="102" x2="110" y2="102" stroke="#38bdf8" stroke-width="2" stroke-linecap="round"/>
    <line x1="86" y1="120" x2="108" y2="120" stroke="#38bdf8" stroke-width="1.8" stroke-linecap="round" opacity="0.7"/>
    <line x1="84" y1="138" x2="108" y2="138" stroke="#38bdf8" stroke-width="2" stroke-linecap="round"/>
    <line x1="88" y1="156" x2="106" y2="156" stroke="#38bdf8" stroke-width="1.8" stroke-linecap="round" opacity="0.7"/>
  </g>

  <!-- Rung endpoints — lit nodes -->
  <g filter="url(#glow)">
    <circle cx="82" cy="30" r="2.8" fill="#38bdf8" opacity="0.95"/>
    <circle cx="112" cy="30" r="2.8" fill="#38bdf8" opacity="0.95"/>
    <circle cx="70" cy="66" r="3.2" fill="#38bdf8"/>
    <circle cx="122" cy="66" r="3.2" fill="#38bdf8"/>
    <circle cx="84" cy="102" r="2.8" fill="#38bdf8" opacity="0.95"/>
    <circle cx="110" cy="102" r="2.8" fill="#38bdf8" opacity="0.95"/>
    <circle cx="84" cy="138" r="2.8" fill="#38bdf8" opacity="0.9"/>
    <circle cx="108" cy="138" r="2.8" fill="#38bdf8" opacity="0.9"/>
  </g>

  <!-- Neural circuit — branches from rung nodes -->
  <!-- Top right cluster -->
  <g stroke="#38bdf8" fill="none" opacity="0.6" filter="url(#softglow)">
    ...
    <circle cx="152" cy="52" r="3.5" fill="#38bdf8" opacity="0.85"/>
    <circle cx="172" cy="62" r="2.2" fill="#38bdf8" opacity="0.7"/>
    <circle cx="160" cy="38" r="2" fill="#38bdf8" opacity="0.65"/>
    <line x1="172" y1="62" x2="184" y2="54" stroke-width="0.8" opacity="0.4"/>
    <circle cx="184" cy="54" r="1.5" fill="#38bdf8" opacity="0.5"/>
  </g>

  <!-- Top left cluster -->
  <g stroke="#38bdf8" fill="none" opacity="0.55" filter="url(#softglow)">
    ...
    <circle cx="42" cy="52" r="3.5" fill="#38bdf8" opacity="0.85"/>
    <circle cx="24" cy="62" r="2.2" fill="#38bdf8" opacity="0.7"/>
    <circle cx="34" cy="36" r="2" fill="#38bdf8" opacity="0.65"/>
  </g>

  <!-- Bottom right cluster -->
  <g stroke="#38bdf8" fill="none" opacity="0.5" filter="url(#softglow)">
    ...
    <circle cx="140" cy="152" r="3.5" fill="#38bdf8" opacity="0.8"/>
    <circle cx="158" cy="144" r="2.2" fill="#38bdf8" opacity="0.65"/>
    <circle cx="148" cy="168" r="2" fill="#38bdf8" opacity="0.6"/>
  </g>
  ...
  <g stroke="#38bdf8" fill="none" opacity="0.5" filter="url(#softglow)">
    ...
    <circle cx="54" cy="152" r="3.5" fill="#38bdf8" opacity="0.8"/>
    <circle cx="36" cy="144" r="2.2" fill="#38bdf8" opacity="0.65"/>
  </g>

  <!-- Central pulse — intelligence core -->
  <circle cx="97" cy="96" r="10" fill="#38bdf8" opacity="0.06"/>
  <circle cx="97" cy="96" r="6"  fill="#38bdf8" opacity="0.15"/>
  <circle cx="97" cy="96" r="3"  fill="#ffffff" opacity="0.85" filter="url(#glow)"/>

  <!-- Crosshair at centre -->
  <g stroke="#38bdf8" stroke-width="0.6" opacity="0.3">
...
def score_variant_ml(
    ...
    if prob is None:
        # Rule-based fallback — same logic as before but normalised
        score = (clinvar_stars*2 + is_lof_intol*3 + cons_score +
                 (2 if gnomad_af==0 else 1.5 if gnomad_af<0.0001 else 0) +
                 (2 if am_score>=0.7 and "missense" in consequence.lower() else 0) +
                 int(plddt>=70) + (2 if cv_density>10 else 1 if cv_density>5 else 0))
        prob = min(0.99, max(0.01, score / 18.0))
...
def format_acmg_badge(criteria: dict) -> str:
    """Return HTML for ACMG criteria badges shown in protein header."""
    met = [(k, v[1]) for k, v in criteria.items()
           if not k.startswith("_") and v[0]]
    if not met:
        return ""
    color_map = {"very_strong":"#ff2d55","strong":"#ff8c42","moderate":"#ffd60a","supporting":"#22c55e"}
    badges = "".join(
        f"<span style='background:{color_map.get(strength,"#3a6080")}18;"
        f"color:{color_map.get(strength,"#3a6080")};border:1px solid {color_map.get(strength,"#3a6080")}33;"
        f"padding:2px 8px;border-radius:5px;font-size:.65rem;font-weight:700;margin:1px;display:inline-block;'>"
        f"{crit}</span>"
        for crit, strength in met
    )
    return f"<div style='margin:.4rem 0;'>{badges}</div>"
...
def assess_gpcr_piggybacking(p, cv, gi_data):
    ...
    if is_gpcr and has_tm and n_germline_path >= 3 and n_named_conditions >= 2:
        ...
        return {
            "type": "PIGGYBACK",
            "label": " PIGGYBACK PROTEIN — GPCR-associated but NOT an independent disease driver",
            "colour": "#ff8c42",
            "confidence": "HIGH",
            "reasoning": (
...
def render_oncology_workspace():
    ...
    for ci, (cname, cd) in enumerate(CDATA.items()):
        with card_cols[ci]:
            ...
            bg = f"{cd['clr']}12" if is_sel else "#020617"
    ...
    with output_col:
        ...
        if tx:
            st.markdown(f"<div style='background:#020617;border:2px solid #22c55e;border-left:5px solid #22c55e;border-radius:0 10px 10px 0;padding:10px 14px;margin-bottom:.6rem;'>"
        ...
        st.markdown(f"<div style='margin-top:.5rem;display:flex;gap:8px;'>"
            f"<div style='flex:1;background:#020617;border:1px solid #ff8c4222;border-radius:7px;padding:6px 9px;'>"
            ...
            + f"</div><div style='flex:1;background:#020617;border:1px solid #22c55e22;border-radius:7px;padding:6px 9px;'>"
...
<style>body{margin:0;background:#020617;overflow:hidden;font-family:Inter,sans-serif;}
...
const P={
 ...
 SCN1A:{x:.10,y:.57,r:11,c:"#38bdf8",label:"Nav1.1",fn:"Na⁺ channel — interneurons",dis:"Dravet (LoF) · GEFS+ (GoF)"},
 KCNQ2:{x:.16,y:.65,r:10,c:"#38bdf8",label:"Kv7.2 M-channel",fn:"AIS repolarisation",dis:"GoF: neonatal epilepsy; LoF: encephalopathy"},
...
function draw(){
 ...
 bg.addColorStop(0,'#020617');bg.addColorStop(1,'#020b1a');
 ...
 x.fillStyle='#38bdf833';x.font='8px Inter';x.textAlign='center';
...
draw();
        ...
        DMAP = {
            ...
            "KCNQ2 encephalopathy":{"genes":["KCNQ2"],"tx":"Carbamazepine/phenobarb (LoF) · XEN496 Kv7 opener (Phase III) · Avoid Na-blockers in GoF","clr":"#38bdf8"},
        ...
        for ch, protein, cclr, detail in CHANS:
            ...
            with cols_ch[0]:
                st.markdown(f"<div style='background:#020617;border:2px solid {cclr};border-radius:9px;padding:.6rem;text-align:center;'>"
            ...
            with cols_ch[1]:
                st.markdown(f"<div style='background:#020617;border-left:3px solid {cclr};padding:8px 12px;border-radius:0 8px 8px 0;font-size:.73rem;color:#4a7090;line-height:1.7;white-space:pre-line;'>{detail}</div>", unsafe_allow_html=True)
...
def render_oncology_panel(gene, pdata, cv, scored, gi, gnomad, ot_data, am_scores, string_data, patient_data):
    ...
    if any(k in gene for k in ["EGFR","ALK","ROS1","MET","BRAF","RET","NTRK"]):
        strategies.append(("Targeted kinase inhibitor", "#38bdf8", f"{gene} = oncogenic kinase driver. 1st line: EGFR→osimertinib, ALK→alectinib, BRAF→dabrafenib+trametinib. Match specific exon/codon variant to approved drug."))
    ...
    if len(cancer_diseases) >= 2:
        strategies.append(("ctDNA liquid biopsy monitoring", "#38bdf8", f"{gene} somatic variants detectable in plasma cell-free DNA. Panel includes top {min(3,n_crit)} CRITICAL variants. Monitor every 8 weeks for MRD and early resistance emergence."))
    if not strategies:
    ...
    for sname, sclr, sdesc in strategies:
        st.markdown(
            f"<div style='background:#020617;border:1px solid {sclr}33;border-left:3px solid {sclr};"
...
def render_microbiome_page():
    ...
    if _mic_mode == " Annotation Engine":
        ...
        st.markdown(
            "<div style='background:#020617;border:1px solid #071828;border-radius:10px;padding:10px 14px;margin-bottom:.8rem;'>"
        ...
        with col_ann2:
            st.markdown(
                "<div style='background:#020617;border:1px solid #071828;border-radius:8px;padding:9px 12px;'>"
                "<div style='color:#38bdf8;font-size:.74rem;font-weight:700;margin-bottom:5px;'>Supported vague terms</div>"
        ...
        if run_ann and vague_input:
            ...
            if result_text:
                ...
                with col_a:
                    st.markdown(
                        f"<div style='background:#020617;border:1px solid #22c55e22;border-radius:8px;padding:9px 12px;height:100%;'>"
    ...
    # ── Tab 2: Taxonomy Intelligence ─────────────────────────────────────────
        ...
        for genus, info in list(display_taxa.items())[:8]:
            ...
            with st.expander(f"{info['full_name']}  ·  {info['phylum']}", expanded=(len(display_taxa)==1)):
                ...
                with col_t1:
                    st.markdown(
                        f"<div style='background:#020617;border-left:3px solid {clr};padding:8px 12px;border-radius:0 8px 8px 0;margin-bottom:7px;'>"
                ...
                with col_t2:
                    st.markdown(
                        f"<div style='background:#020617;border:1px solid {clr}33;border-radius:8px;padding:9px 12px;'>"
                    ...
                    components.html(f"""
<style>body{{margin:0;background:#020617;display:flex;align-items:center;justify-content:center;height:100px;}}</style>
...
dr();
    ...
    # ── Tab 3: Host-Microbe Interactions ─────────────────────────────────────
        ...
        for iname, receptor, iclr, desc in interactions:
            st.markdown(
                f"<div style='background:#020617;border:1px solid {iclr}22;border-radius:9px;padding:9px 12px;margin:.3rem 0;display:flex;gap:10px;'>"
                ...
                f"<div style='color:#38bdf8;font-size:.69rem;font-weight:600;'>{receptor}</div>"
                f"</div>"
...
def render_pharma_workspace():
    ...
    st.markdown("""
    <div style='background:linear-gradient(135deg,#000810,#001520);border:1px solid #38bdf833;
      border-radius:14px;padding:1.1rem 1.4rem;margin-bottom:1rem;'>
      <div style='font-size:1.3rem;font-weight:800;color:#38bdf8;margin-bottom:4px;'>
    ...
    with ph1:
        ...
        for day, detail in steps_filamin:
            st.markdown(f"<div style='display:flex;gap:8px;padding:4px 0;border-bottom:1px solid #050e18;'><span style='color:#38bdf8;font-size:.68rem;font-weight:700;min-width:75px;'>{day}</span><span style='color:#3a6080;font-size:.72rem;line-height:1.55;'>{detail}</span></div>", unsafe_allow_html=True)
        st.markdown("<a href='https://pubmed.ncbi.nlm.nih.gov/26124276/' target='_blank' style='color:#38bdf8;font-size:.7rem;'>PMID:26124276 ↗</a>", unsafe_allow_html=True)
    ...
    with ph2:
        ...
        for rule, criteria, note in admet:
            st.markdown(f"<div style='display:flex;gap:6px;padding:3px 0;border-bottom:1px solid #050e18;'><span style='color:#38bdf8;font-size:.67rem;min-width:120px;font-weight:600;'>{rule}</span><span style='color:#3a6080;font-size:.67rem;'>{criteria} <span style='color:#1e4060;'>({note})</span></span></div>", unsafe_allow_html=True)

    sh("", "Drug Development Timeline")
    timeline = [
        ("Target ID + Validation", "Year 0–1", "#38bdf8", "Genomic Integrity Score, ClinVar P/LP, AlphaMissense, pLI, OpenTargets tractability. Validate with variant biochemical assay (WT vs P/LP)."),
...
def render_molbio_workspace():
    ...
    <div style='background:linear-gradient(135deg,#080400,#120800);border:1px solid #f9731633;
      ...
      <div style='font-size:1.3rem;font-weight:800;color:#fb923c;margin-bottom:4px;'>
         Molecular Biology Platform</div>
...
# Inject the user-supplied Anthropic key from session_state into JS.
# IMPORTANT: we deliberately do NOT inject st.secrets here — that would leak
...
def render_onboarding():
    ...
    for i, s in enumerate(STEPS, 1):
        ...
        c = "#38bdf8" if active else "#34d399" if done else "#1e2d3f"
        tc = "var(--text)" if active else "#34d399" if done else "var(--text3)"
...
RESEARCH_DOMAINS = {
    ...
    "Oncology": {
        "icon": "activity", "color": "#f43f5e", "color2": "#fb7185",
    ...
    "Pharmaceuticals": {
        "icon": "pill", "color": "#38bdf8", "color2": "#38bdf8",
    ...
    "Microbiome": {
        "icon": "atom", "color": "#22c55e", "color2": "#4ade80",
    ...
    "Molecular Biology": {
        "icon": "dna", "color": "#f97316", "color2": "#fb923c",
    ...
    "Rare Disease": {
        "icon": "shield", "color": "#c084fc", "color2": "#d8b4fe",
...
if not st.session_state.get("ob_complete"):
    ...
    st.markdown(
        ...
        f"<img src='data:image/png;base64,{LOGO_B64}' style='width:38px;height:38px;object-fit:contain;'>"
...
if not st.session_state.get("ob_complete"):
    ...
    st.markdown(
        ...
        f"<img src='data:image/png;base64,{LOGO_B64}' style='width:36px;height:36px;object-fit:contain;'>"
        f"<div>"
...
.dom-title{
  ...
  background:linear-gradient(90deg,#38bdf8 0%,#6366f1 35%,#f43f5e 65%,#38bdf8 100%);
...
.dom-card{
  ...
  border:1px solid transparent;background:#020617;}
.dom-card::before{
...
.dom-card .icon{margin-bottom:.5rem;display:block;line-height:0;}
...
""", unsafe_allow_html=True)
    ...
    DOMAIN_STYLES = {
        ...
        "Pharmaceuticals": {
            "icon":"pill","color":"#38bdf8","color2":"#7dd3fc",
            "grad":"linear-gradient(135deg,rgba(56,189,248,.14),rgba(56,189,248,.04))",
            "shadow":"0 8px 32px rgba(56,189,248,.25)",
        ...
        "Rare Disease": {
            "icon":"shield","color":"#a78bfa","color2":"#c4b5fd",
            "grad":"linear-gradient(135deg,rgba(167,139,250,.12),rgba(167,139,250,.04))",
            "shadow":"0 8px 32px rgba(167,139,250,.25)",
            "border":"rgba(167,139,250,.5)",
            "tags":"Mendelian variants · Pedigree analysis · ACMG criteria · Patient cohorts"
        },
    }
    ...
    for i, dk in enumerate(domain_keys[:3]):
        ...
        with r1[i]:
            st.markdown(
                ...
                f"'>"
                f"<span class='icon'>{svg_icon(ds['icon'], size=36, color=ds['color2'])}</span>"
...
with st.sidebar:
    ...
    _rd_clr_sb = _rd_meta_sb.get("color","#38bdf8")
    _rd_icon_sb = _rd_meta_sb.get("icon","")
    st.markdown(
        f"<div style='text-align:center;padding:.3rem 0 .5rem;'>"
        f"<div style='display:flex;justify-content:center;margin-bottom:4px;'>{svg_icon(_rd_icon_sb, size=22, color=_rd_clr_sb) if _rd_icon_sb else ''}</div>"
...
if not st.session_state.get("pdata") and not st.session_state.get("csv_triage_active"):
    ...
    st.markdown(
        ...
        f"Use the sidebar search — gene symbol (TP53, BRCA1, FLNC) or UniProt accession (P04637). "
        f"{'Suggested from your lab: <b style=color:#38bdf8;>' + _first_prot + '</b>.' if _first_prot else 'Try: ' + _suggest + ' to explore.'}"
        f"</div></div></div>",
        unsafe_allow_html=True,
    )
...
if _rd_final == "Oncology":
    ...
    if not _pdata_f and not search:
        ...
        with st.expander(" Molecular Biology Workspace", expanded=False):
            render_molbio_workspace()
...
def _render_enhanced_signals():
    ...
    try:
        _cf   = list(st.session_state.get("conflicts") or [])
        _acmg = dict(st.session_state.get("acmg_auto") or {})
        _dom  = dict(st.session_state.get("domain_ctx") or {})
        _ml   = dict(st.session_state.get("ml_result") or {})
        _cg   = dict(st.session_state.get("clingen") or {})
    except Exception:
        return
    ...
    if _cg_classes:
        ...
        for c in _cg_classes[:3]:
            cls_name = str(c.get("classification",""))
            dis_name = str(c.get("disease",""))[:28]
            clr = _VCOL.get(cls_name, "#3a6080")
            parts.append(
                f"<span style='background:{clr}18;color:{clr};border:1px solid {clr}44;"
                f"padding:2px 9px;border-radius:5px;font-size:.66rem;font-weight:700;"
                f"margin:2px;display:inline-block;'>ClinGen {cls_name}: {dis_name}</span>"
            )
        if parts:
            st.markdown(
                "<div style='margin:.3rem 0;'><span style='color:#1e4060;font-size:.64rem;"
                "text-transform:uppercase;'>ClinGen validity: </span>"
                + "".join(parts) + "</div>",
                unsafe_allow_html=True
            )
    ...
    for item in _cf:
        ...
        clr = item.get("color", "#3a6080")
        typ = item.get("type", "").replace("_", " ")
        sev = item.get("severity", "")
        msg = item.get("message", "")
        st.markdown(
            f"<div style='background:{clr}0c;border-left:3px solid {clr};"
            f"border-radius:0 8px 8px 0;padding:6px 12px;margin:.25rem 0;'>"
            f"<span style='color:{clr};font-size:.7rem;font-weight:700;'> {typ} [{sev}]</span>"
            f"<div style='color:#3a6080;font-size:.72rem;margin-top:1px;'>{msg}</div></div>",
            unsafe_allow_html=True
        )

    # ACMG badges
    if _acmg:
        _met = [(k, v[1]) for k, v in _acmg.items()
                if not k.startswith("_") and isinstance(v, tuple) and len(v) > 1 and v[0]]
        if _met:
            _cmap = {"very_strong":"#ff2d55","strong":"#ff8c42",
                     "moderate":"#ffd60a","supporting":"#22c55e"}
            badges = "".join(
                f"<span style='background:{_cmap.get(s,'#3a6080')}18;"
                f"color:{_cmap.get(s,'#3a6080')};border:1px solid {_cmap.get(s,'#3a6080')}33;"
                f"padding:2px 8px;border-radius:5px;font-size:.64rem;font-weight:700;"
                f"margin:1px;display:inline-block;'>{c}</span>"
                for c, s in _met
            )
            st.markdown(
                "<div style='margin:.3rem 0;'>"
                "<span style='color:#1e4060;font-size:.64rem;text-transform:uppercase;'>"
                "Auto-ACMG: </span>" + badges + "</div>",
                unsafe_allow_html=True
            )

    # Domain hotspot
    top_dom = _dom.get("top_hotspot_domain")
    if top_dom and isinstance(top_dom, dict) and top_dom.get("is_hotspot") and top_dom.get("plp_count", 0) >= 2:
        name = top_dom.get("name","")
        s = top_dom.get("start",0); e = top_dom.get("end",0)
        n = top_dom.get("plp_count",0); d = top_dom.get("density",0)
        st.markdown(
            f"<div style='background:#ff2d550a;border:1px solid #ff2d5520;border-radius:7px;"
            f"padding:5px 10px;margin:.3rem 0;font-size:.72rem;'>"
            f"<span style='color:#ff2d55;font-weight:700;'>Hotspot: </span>"
            f"<span style='color:#3a6080;'>{name} aa{s}–{e} · {n} P/LP · density {d:.1f}%</span></div>",
            unsafe_allow_html=True
        )

    # ML pursuit probability
    prob = _ml.get("probability")
    if prob is not None:
        src  = "ML" if _ml.get("source") == "ml_model" else "Rules"
        pc   = "#22c55e" if prob>=0.75 else "#ffd60a" if prob>=0.5 else "#ff8c42" if prob>=0.25 else "#ff2d55"
        st.markdown(
            f"<div style='display:flex;align-items:center;gap:8px;margin:.3rem 0;'>"
            f"<span style='color:#1e4060;font-size:.64rem;text-transform:uppercase;'>Pursuit probability:</span>"
            f"<div style='flex:1;background:#050e18;border-radius:3px;height:5px;'>"
            f"<div style='background:{pc};height:5px;border-radius:3px;width:{int(prob*100)}%;'></div></div>"
            f"<span style='color:{pc};font-size:.72rem;font-weight:700;'>{prob:.1%}</span>"
...
def render_chemical_backbone(seq, cv_variants, phospho_sites, binding_sites, gene, pLI=0.0):
    ...
    st.markdown(f"""
<div style='background:#020617;border:1px solid #071828;border-radius:10px;padding:11px 16px;
  ...
  <div><div style='color:#3a6080;font-size:.6rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;'>Molecular Formula</div>
    <div style='color:#38bdf8;font-size:.9rem;font-weight:700;font-family:JetBrains Mono,monospace;'>{mol_html}</div></div>
...
<style>*{{margin:0;padding:0;box-sizing:border-box;}}body{{background:#020617;overflow:hidden;}}
canvas{{display:block;}}
...
.btn{{background:#020617;border:1px solid #071828;color:#3a6080;border-radius:6px;
  padding:4px 9px;font-size:10px;cursor:pointer;transition:all .12s;font-family:Inter,sans-serif;}}
.btn:hover,.btn.on{{border-color:rgba(0,229,255,.35);color:#38bdf8;}}
...
const WIN=Math.max(18,Math.floor((W-80)/36));
...
function gpos(i,n){{
  const sp=Math.max(36,Math.min(56,(W-80)/n));
  const xb=40+i*sp;
  const yb=H/2+(i%2===0?-56:56);
  return{{x:xb,y:yb,sp}};
}}
...
function drawRes(r,pos,n,isH){{
  ...
  const rv=Math.max(10,Math.min(20,sp*0.42));
...
function draw(){{
  ...
  if(sig){{
    ...
    x.beginPath();x.arc(sp.x,sp.y,6,0,Math.PI*2);x.fillStyle='#38bdf8';x.fill();
    sigT+=0.005;
  }}
  x.restore();
...
cv.addEventListener('mousemove',e=>{{
  ...
  if(hov!==null){{
    ...
    el.innerHTML=`<b style="color:#38bdf8">Pos ${{res.pos}} — ${{res.aa}} (${{res.name}})</b><br>`
...
if _pdata_f and _gene_f:
    ...
    if seq_hook:
        with tab8:
            st.markdown("<hr class='dv'>", unsafe_allow_html=True)
            sh("", f"Chemical Backbone — {_gene_f} Protein Chain (Interactive)")
            st.markdown(
...
def render_oncology_workspace():
    ...
    for rname, rclr, rdesc in recs:
        st.markdown(f"<div style='background:#020617;border-left:3px solid {rclr};border-radius:0 8px 8px 0;padding:9px 13px;margin:.35rem 0;'><div style='color:{rclr};font-weight:700;font-size:.82rem;'>{rname}</div><div style='color:#3a6080;font-size:.77rem;line-height:1.6;margin-top:2px;'>{rdesc}</div></div>", unsafe_allow_html=True)
    ...
    for step, title, molecular, regulators, clr, intervention in cascade:
        with st.expander(f"{step} — {title}"):
            ...
            with cc2:
                st.markdown(f"<div style='background:#020617;border:1px solid {clr}33;border-left:3px solid {clr};border-radius:0 8px 8px 0;padding:8px 11px;font-size:.73rem;color:#4a7090;line-height:1.6;'><b style='color:{clr};'>Clinical relevance:</b> {intervention}</div>", unsafe_allow_html=True)
...
def render_neuroscience_workspace():
    ...
    for circuit_name, circuit_data in NEURAL_CIRCUITS.items():
        ...
        with st.expander(f"{circuit_name}", expanded=False):
            ...
            for pi, (prot, pdesc) in enumerate(circuit_data["proteins"].items()):
                with pr_cols[pi % 2]:
                    st.markdown(f"<div style='background:#020617;border:1px solid {clr}22;border-radius:8px;padding:7px 10px;margin:3px 0;'><div style='color:{clr};font-weight:700;font-size:.78rem;'>{prot}</div><div style='color:#3a5060;font-size:.7rem;line-height:1.55;margin-top:2px;'>{pdesc}</div></div>", unsafe_allow_html=True)
    ...
    # ── Disease → protein map ─────────────────────────────────────────────
    st.markdown("<hr class='dv'>", unsafe_allow_html=True)
    sh("", "Disease → Protein Map — Click to Analyse")
    DISEASE_PROTS = {
...
def render_pharma_workspace():
    ...
    st.markdown("<div style='color:#38bdf8;font-size:1.1rem;font-weight:800;margin-bottom:.4rem;'> Drug Discovery Workspace</div>", unsafe_allow_html=True)
    ...
    if ph_mode == "★ GPCR Filamin Checker":
        ...
        if gpcr_q:
            ...
            if is_g:
                st.markdown(f"<div style='background:#020617;border:2px solid #22c55e;border-radius:10px;padding:12px 16px;margin:.4rem 0;'>"
                    ...
                    f"<div style='color:#38bdf8;font-size:.73rem;font-weight:700;margin-bottom:4px;'>Recommended assay sequence:</div>"
        ...
        for day, detail in [("Day 1","Seed HEK293 (1M/well) + GPCR expression. Untransfected control + β2AR positive control."),("Day 2","Serum-starve 2h. Agonist dose-response 0.1nM–10μM, 15min, 37°C. H89 10μM (30min pre) as PKA control."),("Day 3","RIPA lysis + PhosSTOP + protease inhibitors. IP: anti-FLNA Ig21 (Millipore 3/F9, 1:200) overnight."),("Day 4","Elute → SDS-PAGE → WB: anti-pSer2152 (CST #4761). Strip → re-probe total FLNA. 4-PL EC50 curve."),("Read","H89 abolishes signal = PKA confirmed. pSer2152 EC50 < cAMP EC50 = receptor-proximal (expected).")]:
            st.markdown(f"<div style='display:flex;gap:8px;padding:4px 0;border-bottom:1px solid #050e18;'><span style='color:#38bdf8;font-size:.7rem;font-weight:700;min-width:45px;'>{day}</span><span style='color:#3a6080;font-size:.72rem;line-height:1.55;'>{detail}</span></div>", unsafe_allow_html=True)
        st.markdown("<a href='https://pubmed.ncbi.nlm.nih.gov/26124276/' target='_blank' style='color:#38bdf8;font-size:.7rem;'> Bhatt et al. 2015 — PMID:26124276 ↗</a>", unsafe_allow_html=True)
...
def render_molbio_workspace():
    ...
    st.markdown("<div style='color:#fb923c;font-size:1.1rem;font-weight:800;margin-bottom:.4rem;'> Molecular Biology Workspace</div>", unsafe_allow_html=True)
    ...
    if mb_mode == " Kinase → Substrate":
        ...
        for sub, site, detail in kd2["subs"]:
            ...
            with s1:
                st.markdown(f"<div style='background:#020617;border:1px solid {clr5}33;border-radius:8px;padding:.5rem;text-align:center;'>"
        ...
        if have_protein == "No — bioinformatics only":
            st.markdown("<div style='background:#020617;border-left:3px solid #f97316;padding:9px 13px;border-radius:0 9px 9px 0;color:#3a6080;font-size:.75rem;line-height:1.7;'>"
            ...
            st.markdown(f"<div style='background:#020617;border-left:4px solid #f97316;padding:10px 14px;border-radius:0 10px 10px 0;color:#3a6080;font-size:.77rem;line-height:1.65;margin-top:.5rem;'>"
...
# ── Lab chatbot ──────────────────────────────────────────────────────────────
# ── Lab profile that drives all workspace mutations ───────────────────────────
LAB_PROFILE_DEFAULTS = {
    "lab_name": "",
    "lab_pi": "",
    "lab_focus": "",           # free text: "epilepsy, SCN1A variants, iPSC neurons"
    "lab_domain": "",          # Neuroscience / Oncology / etc.
    "lab_proteins": [],        # ["SCN1A", "SCN2A", "KCNQ2"]
    "lab_diseases": [],        # ["Dravet syndrome", "GEFS+"]
    "lab_techniques": [],      # ["patch-clamp", "iPSC", "Western blot"]
    "lab_goal": "",            # therapeutic / mechanism / clinical / basic
    "lab_budget": "medium",    # low / medium / high
    "lab_team_size": "",       # "5 postdocs + 2 students"
    "lab_model_organism": "",  # mouse / zebrafish / iPSC / patient samples
...
DOMAIN_KEYWORDS = {
    "Neuroscience":       ["neuro","synapse","epilepsy","seizure","brain","neuron","gaba","glutamate","ion channel","scn","kcnq","cacna","shank","syngap","autism","asd","alzheimer","parkinson","als","dementia","dravet","rett","lrrk2"],
    "Oncology":           ["cancer","tumour","tumor","oncology","brca","kras","tp53","chemotherapy","immunotherapy","metastasis","carcinoma","lymphoma","melanoma","glioma","leukaemia","solid tumour","ctdna","pdx","crc","nsclc"],
    "Pharmaceuticals":    ["drug","gpcr","pharma","receptor","agonist","antagonist","compound","lead","clinical trial","admet","hts","screening","fda","ind","target","small molecule","antibody","biologic"],
    "Microbiome":         ["microbiome","microbiota","metagenomics","gut","bacteria","16s","shotgun","bgc","annotation","butyrate","scfa","dysbiosis","phage","microbe","probiotic","metabolomics"],
    "Molecular Biology":  ["kinase","phosphorylation","ptm","structural","cryo-em","alphafold","hdx","saxs","western blot","pull-down","biochemistry","mechanism","domain","interaction","signalling"],
    "Rare Disease":       ["rare disease","mendelian","vus","variant","wes","wgs","exome","genome","acmg","clinvar","gnomad","patient","family","de novo","inheritance","phenotype","hpo","diagnosis"],
}

GOAL_MAP = {
    "therapeutic":  " Identify therapeutic targets",
    "drug":         " Drug discovery & development",
    "mechanism":    " Understand disease mechanism",
    "biomarker":    " Biomarker identification",
    "basic":        " Basic research / functional characterisation",
    "clinical":     " Clinical variant interpretation",
    "experiment":   " Experimental pathway prioritisation",
}

SYSTEM_PROMPT = """You are the Protellect Lab Setup Assistant. Your ONLY job is to interview a researcher and extract structured information about their lab so you can configure Protellect perfectly for them.

You must extract ALL of these in a natural conversation:
1. Lab name and PI name
2. Research focus (diseases, proteins, pathways they study)
3. Their main scientific goal (drug discovery / mechanism / clinical interpretation / basic research / biomarker)
4. Key proteins or genes they work on
5. Diseases they study
6. Experimental techniques they use (patch-clamp, iPSC, CRISPR, etc.)
7. Model organisms / patient material
8. Team size and budget tier (helps calibrate sensitivity thresholds)

Rules:
- Ask maximum 2 questions at a time. Be conversational and warm.
- After 4-5 exchanges you should have enough — then output the JSON configuration block.
- When you have enough info, output EXACTLY this format on its own line:

PROTELLECT_CONFIG:{"domain":"Neuroscience","goal":"mechanism","proteins":["SCN1A","SCN2A"],"diseases":["Dravet syndrome"],"techniques":["patch-clamp","iPSC"],"sensitivity":45,"lab_name":"Smith Lab","pi":"Dr Smith","focus":"ion channel epilepsies","budget":"medium","organism":"iPSC neurons + mouse"}

Valid domains: Neuroscience, Oncology, Pharmaceuticals, Microbiome, Molecular Biology, Rare Disease
Valid goals: therapeutic, drug, mechanism, biomarker, basic, clinical, experiment
Sensitivity: 30=broad/exploratory, 50=balanced, 70=strict/clinical

- Keep responses SHORT. 2-4 sentences max before asking the next question.
- Be enthusiastic and specific — reference their proteins back to them.
- After outputting the config, give a brief friendly summary of what you've set up."""


def extract_config_from_response(text: str) -> dict | None:
    """Pull the PROTELLECT_CONFIG JSON out of the AI response."""
    match = re.search(r'PROTELLECT_CONFIG:(\{.*?\})', text, re.DOTALL)
    if match:
        try:
...
def apply_config_to_workspace(cfg: dict) -> list[str]:
    """
    Apply the extracted config to Protellect session state.
    ...
    valid_domains = ["Neuroscience","Oncology","Pharmaceuticals","Microbiome","Molecular Biology","Rare Disease"]
    if domain in valid_domains:
        # Full domain switch — clear ALL protein state first
    ...
    # ── 2. Research goal ──────────────────────────────────────────────────────
    goal_key = cfg.get("goal","")
    if goal_key in GOAL_MAP:
        st.session_state["goal_label"] = GOAL_MAP[goal_key]
        changes.append(f"Research goal → **{GOAL_MAP[goal_key]}**")

    # ── 3. Sensitivity threshold ──────────────────────────────────────────────
    sensitivity = cfg.get("sensitivity", 50)
    try:
        sensitivity = int(sensitivity)
        st.session_state["sensitivity"] = max(10, min(90, sensitivity))
        label = "strict (clinical)" if sensitivity >= 65 else "balanced" if sensitivity >= 40 else "broad (exploratory)"
        changes.append(f"Sensitivity → **{sensitivity}** ({label})")
    except Exception:
        pass

    # ── 4. Lab profile storage ────────────────────────────────────────────────
    st.session_state["lab_name"]      = cfg.get("lab_name","")
    st.session_state["lab_pi"]        = cfg.get("pi","")
    st.session_state["lab_focus"]     = cfg.get("focus","")
    st.session_state["lab_proteins"]  = cfg.get("proteins",[])
    st.session_state["lab_diseases"]  = cfg.get("diseases",[])
    st.session_state["lab_techniques"]= cfg.get("techniques",[])
    st.session_state["lab_budget"]    = cfg.get("budget","medium")
    st.session_state["lab_model_organism"] = cfg.get("organism","")
    st.session_state["lab_domain"]    = cfg.get("domain","")
    st.session_state["lab_goal"]      = cfg.get("goal","")
    st.session_state["lab_configured"] = True
    st.session_state["lab_setup_complete"] = True

    # ── 5. Pre-populate protein search if one protein given ───────────────────
    proteins = cfg.get("proteins",[])
    if proteins and not st.session_state.get("pdata"):
        top = proteins[0].strip().upper()
        if top:
            st.session_state["protein_query_val"] = top
            changes.append(f"Protein search pre-loaded → **{top}**")

    # ── 6. Update sidebar disease search ─────────────────────────────────────
    diseases = cfg.get("diseases",[])
    if diseases:
        st.session_state["disease_search"] = diseases[0]
        changes.append(f"Disease context → **{diseases[0]}**")

    return changes


def _get_anthropic_key() -> str:
    """Resolve Anthropic API key from (1) st.secrets, (2) env var, (3) session state."""
    try:
...
def render_lab_chatbot():
    """Render the floating lab configuration chatbot."""
    # Init defaults
    for k, v in LAB_PROFILE_DEFAULTS.items():
        if k not in st.session_state:
            st.session_state[k] = v
    ...
    btn_label = (
        f" {lab_name or 'Lab'} configured" if is_done
        else " Configure for your lab"
    )
    btn_clr   = "#22c55e" if is_done else "#c084fc"

    st.sidebar.markdown(
        f"<div style='height:1px;background:#071828;margin:.5rem 0;'></div>",
        unsafe_allow_html=True
    )

    col_btn, col_reset = st.sidebar.columns([3,1])
    with col_btn:
        if st.button(btn_label, key="lab_chat_toggle", use_container_width=True):
            st.session_state["lab_chat_open"] = not is_open
            st.rerun()
    with col_reset:
        if is_done and st.button("↺", key="lab_reset", help="Reset lab profile"):
            for k, v in LAB_PROFILE_DEFAULTS.items():
                st.session_state[k] = v
            st.rerun()
    ...
    if not is_open:
        if is_done:
            dom = st.session_state.get("lab_domain","")
            focus = st.session_state.get("lab_focus","")[:50]
            st.sidebar.markdown(
                f"<div style='background:#0a1a0a;border:1px solid #22c55e22;border-radius:8px;"
                f"padding:6px 10px;margin-top:4px;'>"
                f"<div style='color:#22c55e;font-size:.67rem;font-weight:700;'>✓ {dom} workspace</div>"
                f"<div style='color:#1e4060;font-size:.65rem;'>{focus}</div></div>",
                unsafe_allow_html=True
            )
        return

    # ── Chat panel (rendered in sidebar when open) ────────────────────────────
    st.sidebar.markdown(
        "<div style='background:#050d24;border:1px solid #c084fc22;border-radius:12px;"
        "padding:10px 12px;margin-top:4px;'>",
        unsafe_allow_html=True
    )
    st.sidebar.markdown(
        "<div style='color:#c084fc;font-size:.8rem;font-weight:700;margin-bottom:.4rem;'>"
        " Lab Setup Assistant</div>"
        "<div style='color:#3a1060;font-size:.7rem;margin-bottom:.6rem;'>"
        "Tell me about your lab — I'll configure Protellect for you.</div>",
        unsafe_allow_html=True
    )

    # Show chat history
    for msg in chat_hist[-6:]:  # Show last 6 messages
        role = msg["role"]
        text = msg["content"]
        ...
        st.sidebar.markdown(
            f"<div style='background:{bg};border-radius:8px;padding:7px 9px;margin:.25rem 0;"
            f"font-size:.72rem;color:{clr};line-height:1.55;'>{prefix}{display[:280]}</div>",
            unsafe_allow_html=True
        )

    # Show applied changes if config was just extracted
    if st.session_state.get("_lab_changes_pending"):
        changes = st.session_state.pop("_lab_changes_pending")
        for ch in changes:
            st.sidebar.markdown(
                f"<div style='background:#0a1a0a;border-left:2px solid #22c55e;"
                f"padding:3px 8px;font-size:.67rem;color:#22c55e;margin:2px 0;'>{ch}</div>",
                unsafe_allow_html=True
            )

    # If setup complete, show summary
    if is_done and chat_hist:
        st.sidebar.markdown(
            "<div style='background:#0a1a0a;border:1px solid #22c55e33;border-radius:8px;"
            "padding:7px 10px;margin:.4rem 0;'>"
            "<div style='color:#22c55e;font-size:.7rem;font-weight:700;margin-bottom:3px;'> Workspace configured</div>"
            f"<div style='color:#1e4060;font-size:.66rem;'>Domain: {st.session_state.get('lab_domain','')} · "
            f"Goal: {st.session_state.get('lab_goal','')} · "
            f"Sensitivity: {st.session_state.get('sensitivity',50)}</div></div>",
            unsafe_allow_html=True
        )
        if st.sidebar.button(" Reconfigure workspace", key="lab_reconfig", use_container_width=True):
        ...
        user_input = st.sidebar.text_area(
            "", placeholder="e.g. We study rare epilepsies, mainly SCN1A variants causing Dravet syndrome...",
            height=80, key="lab_chat_input", label_visibility="collapsed"
        )
        if st.sidebar.button("Send →", key="lab_chat_send", type="primary", use_container_width=True):
            if user_input.strip():
                # Add user message
                chat_hist.append({"role":"user","content":user_input.strip()})
                st.session_state["lab_chat_history"] = chat_hist

                # Build messages for API (max last 10 for context window)
                api_messages = chat_hist[-10:]

                # If first message, add greeting from assistant
                if len(chat_hist) == 1:
                    greeting = ("Hi! I'm your Protellect setup assistant. I'll configure the entire "
                               "workspace for your lab in just a few questions. "
                               "What diseases or proteins does your lab focus on? And what's the main goal — "
                               "understanding mechanisms, finding drug targets, or something else?")
                    # Insert greeting before user's first message
                    api_messages = [
                ...
                with st.sidebar:
                    with st.spinner("Thinking..."):
                        reply = call_claude_api(api_messages)

                # Check if config is in reply
                cfg = extract_config_from_response(reply)
                if cfg:
                ...
                st.session_state["lab_chat_history"] = chat_hist
                # Full rerun — loads new domain workspace immediately
                st.rerun()

        # Quick start buttons
        if not chat_hist:
            st.sidebar.markdown(
                "<div style='color:#3a1060;font-size:.66rem;margin:.5rem 0 .2rem;'>Quick start:</div>",
                unsafe_allow_html=True
            )
            quick_options = [
                (" Rare disease / genetics lab", "We're a rare disease genetics lab studying Mendelian disorders using WES/WGS. Our main goal is clinical variant interpretation and identifying pathogenic variants. We work with patient samples and iPSC models."),
                (" Cancer biology lab", "We're a cancer biology lab focusing on solid tumours, mainly KRAS-driven cancers. Our goal is identifying therapeutic targets and understanding resistance mechanisms to targeted therapy."),
                (" Drug discovery team", "We're a pharma drug discovery team working on GPCR targets. Our goal is identifying druggable targets and running ADMET-aware screening campaigns. We have high-throughput capacity."),
                (" Neuroscience / epilepsy lab", "We study genetic epilepsies, mainly sodium channelopathies including SCN1A and SCN2A variants. Our focus is understanding GoF vs LoF mechanisms and finding precision treatment approaches."),
            ]
            for label, msg in quick_options:
                if st.sidebar.button(label, key=f"quick_{label[:10]}", use_container_width=True):
                    chat_hist.append({"role":"user","content":msg})
                    api_messages = [{"role":"user","content":msg}]
                    reply = call_claude_api(api_messages)
                    cfg = extract_config_from_response(reply)
                    if cfg:
                        changes = apply_config_to_workspace(cfg)
                        ...
                        chat_hist.append({"role":"assistant","content":reply})
                    st.session_state["lab_chat_history"] = chat_hist
                    st.session_state["lab_chat_open"] = False  # close chat after quick setup
                    st.rerun()

    st.sidebar.markdown("</div>", unsafe_allow_html=True)

    # ── Popup-chat API key input (optional) ──────────────────────────────
    ...
    with st.sidebar.expander("Popup chat: API key", expanded=False):
        st.caption(
            ...
            "Paste your own Anthropic API key here to enable it. Leave blank to use only the "
            "sidebar assistant (recommended)."
        )
        _ak_in = st.text_input(
            "Anthropic API key (browser-side)",
    ...
    with st.sidebar.expander(f"Lab Library ({len(_lab_papers)} papers)", expanded=False):
        if not _lab_papers and not _lab_focus_sb:
            ...
            if _lab_focus_sb:
                st.caption(f"Focus: {_lab_focus_sb[:80]}{'…' if len(_lab_focus_sb)>80 else ''}")
                    ...
                    if _q:
                        with st.spinner("Searching Semantic Scholar..."):
                            try:
                                ...
                                st.error(f"Could not fetch papers: {str(_ex)[:80]}")
            with colB:
                if st.button("Clear", key="lab_lib_clear", use_container_width=True):
                    st.session_state["ob_papers"] = []
                    st.rerun()
            for _p in _lab_papers[:8]:
                _cc = _p.get("citations", 0)
                _url = _p.get("url") or "#"
                _yr = _p.get("year","")
                st.markdown(
                    f"<div style='background:var(--surface);border:1px solid var(--border);"
                    f"border-radius:6px;padding:6px 8px;margin:.25rem 0;'>"
                    f"<a href='{_url}' target='_blank' style='color:var(--text);font-size:.74rem;"
                    f"font-weight:600;text-decoration:none;line-height:1.35;display:block;'>"
                    f"{_p.get('title','')[:80]}{'…' if len(_p.get('title',''))>80 else ''}</a>"
                    f"<div style='color:var(--text3);font-size:.66rem;margin-top:2px;'>"
                    f"{_p.get('authors','')[:50]} · {_yr} · {_cc:,} cit.</div></div>",
                    unsafe_allow_html=True,
                )
...
_rd_color = _rd_meta.get("color", "#38bdf8")
    ...
    if not ai:
        ...
        if abstracts:
            ...
            for etype, plist in sorted(exp_types.items()):
                st.markdown(
                    f"<div style='color:#38bdf8;font-weight:700;font-size:.9rem;margin:.6rem 0 .3rem;'>{etype} ({len(plist)} papers)</div>",
                    unsafe_allow_html=True,
                )
                for p2 in plist[:3]:
                    st.markdown(
                        f"<div style='background:#020617;border:1px solid #0d2545;border-radius:8px;"
        ...
        if string_data:
            ...
            for si in string_data:
                ...
                rows_s += (
                    ...
                    f"<div style='width:{score_pct}%;height:100%;background:#38bdf8;'></div></div>"
        ...
        if verdict:
            st.markdown(
                ...
                " AI-generated based solely on fetched data. All claims grounded in UniProt, ClinVar, PubMed, gnomAD, STRING sources above.</div>"
                "</div>",
                unsafe_allow_html=True,
            )
        ...
        if exps_done:
            ...
            for e2 in exps_done:
                st.markdown(
                    f"<div style='background:#020617;border:1px solid #0d2545;border-left:3px solid #4a90d9;"
        ...
        if unknowns:
            ...
            for u in unknowns:
                st.markdown(
                    f"<div style='display:flex;gap:8px;background:#020617;border:1px solid #1e3050;"
        ...
        if warnings:
            st.markdown("<hr class='dv'>", unsafe_allow_html=True)
            sh("","Warning Flags from AI Analysis")
            for w in warnings:
                st.markdown(
            ...
            for c_hyp in cure_hyps_ai:
                ...
                with st.expander(f"{c_hyp.get('disease','?')} — {c_hyp.get('approach','?')}", expanded=False):
                    st.markdown(
                        ...
                        f"<div style='background:#050d24;border:1px solid #00c89633;border-radius:8px;padding:.7rem;margin-bottom:.4rem;'>"
        ...
        if st.button(" Regenerate AI Report", key="regen_ai"):
...
with tab6:
    ...
    # ── HEADER ────────────────────────────────────────────────────────────────
    st.markdown("""
    <div style='background:linear-gradient(135deg,#050d24,#08142a);border:1px solid #38bdf818;
    border-radius:14px;padding:1rem 1.4rem;margin-bottom:1rem;display:flex;align-items:center;gap:14px;'>
      <span style='font-size:1.6rem;'></span>
      <div>
        <div style='color:#38bdf8;font-weight:800;font-size:1.05rem;'>Protein Screener & Lab Configurator</div>
    ...
    if _scr_mode == " Protein Screener":
        ...
        if st.session_state.get("screener_running") and st.session_state.get("screener_genes_run"):
            ...
            if _todo:
                ...
                for _g in _todo[:3]:  # Process 3 at a time per rerun
                    _status.markdown(
                        f"<div style='color:#38bdf8;font-size:.8rem;'> Fetching {_g} from ClinVar + gnomAD + UniProt...</div>",
                        unsafe_allow_html=True
                    )
                    try:
        ...
        if _results:
            ...
            <div style='display:flex;gap:1rem;margin:.8rem 0;flex-wrap:wrap;'>
              <div style='background:#050d24;border:1px solid #22c55e22;border-radius:8px;padding:6px 14px;'>
              ...
              <div style='background:#050d24;border:1px solid #ffd60a22;border-radius:8px;padding:6px 14px;'>
              ...
              <div style='background:#050d24;border:1px solid #ff8c4222;border-radius:8px;padding:6px 14px;'>
              ...
              <div style='background:#050d24;border:1px solid #ff2d5522;border-radius:8px;padding:6px 14px;'>
              ...
              <div style='background:#050d24;border:1px solid #3a608022;border-radius:8px;padding:6px 14px;'>
    ...
    # ════════════════════════════════════════════════════════════════════════
        ...
        st.markdown(
            "<div style='background:#050d24;border:1px solid #c084fc22;border-radius:12px;padding:14px 16px;'>",
            unsafe_allow_html=True
        )
        st.markdown(
...
with tab8:
    ...
    is_kin_c = any(
        k in " ".join(kw.get("value","") for kw in pdata.get("keywords",[])).lower()
        for k in ["kinase","phosphotransferase"]
    )
    ...
    if seq_c:
        st.markdown(
            f"<div style='background:#050d24;border:1px solid #38bdf815;border-radius:10px;"
            f"padding:8px 14px;margin-bottom:.8rem;'>"
            f"<b style='color:#38bdf8;font-size:.88rem;'>{gene}</b>"
            f"<span style='color:#3a6080;font-size:.78rem;'> · {len(seq_c):,} aa · "
            f"Chemistry workspace · </span>"
            f"{src_link('UniProt', f'https://www.uniprot.org/uniprotkb/{uid}/entry')}"
            f"</div>",
            unsafe_allow_html=True,
        )

        if _chem_mode == " Chemical Properties":
            sh("", f"Chemical Properties — {gene}")
            ...
            props = [
                ("Molecular Weight", _mw_display, "#38bdf8"),
                ("Length", f"{len(seq_c):,} aa", "#38bdf8"),
            ...
            all_cols = list(cols_r1) + list(cols_r2)
            for col, (lbl, val, clr) in zip(all_cols, props):
                with col:
                    st.markdown(
                        f"<div style='background:#050d24;border:1px solid {clr}22;border-left:3px solid {clr};"
                        f"border-radius:9px;padding:10px 12px;margin-bottom:8px;'>"
                        f"<div style='color:{clr};font-size:1.15rem;font-weight:700;'>{val}</div>"
                        f"<div style='color:#3a6080;font-size:.68rem;margin-top:2px;'>{lbl}</div>"
                        f"</div>",
                        unsafe_allow_html=True,
                    )
            st.markdown("<hr class='dv'>", unsafe_allow_html=True)
            ...
            if is_gpcr_c:
                try:
                    ...
                    bindability.append((" GPCR — premier drug target class", "#38bdf8", f"{_cls}. Orthosteric + allosteric + bitopic sites. {_h8_note}"))
                except Exception:
                    bindability.append((" GPCR target", "#38bdf8", "7-TM receptor — orthosteric + allosteric sites available"))
            if is_kin_c: bindability.append((" Kinase — ATP pocket druggable", "#38bdf8", "Type I/II/III inhibitors + allosteric sites. Check PDB for co-crystal structures with ATP analogues"))
            ...
            aa_counts = {aa: seq_c.count(aa) for aa in "ACDEFGHIKLMNPQRSTVWY"}
            aa_groups = {"Nonpolar":list("GAVILMP"),"Aromatic":list("FYW"),"Polar uncharged":list("STCNQ"),"Positive":list("KRH"),"Negative":list("DE")}
            gcolors = {"Nonpolar":"#4a90d9","Aromatic":"#c084fc","Polar uncharged":"#22c55e","Positive":"#38bdf8","Negative":"#ff2d55"}
            fig_aa = go.Figure()
            for group, aas in aa_groups.items():
                clr = gcolors[group]
                fig_aa.add_trace(go.Bar(
                    x=[aa for aa in aas if aa in aa_counts],
                    y=[round(aa_counts[aa]/len(seq_c)*100,1) for aa in aas if aa in aa_counts],
                    name=group, marker_color=clr, opacity=0.85,
                    hovertemplate="%{x}: %{y:.1f}%<extra></extra>",
                ))
            fig_aa.update_layout(barmode="group", plot_bgcolor="#020617", paper_bgcolor="#020617", font_color="#b0d8f0", font_size=10, xaxis=dict(gridcolor="#0a1828", title="Amino acid"), yaxis=dict(gridcolor="#0a1828", title="% of sequence"), legend=dict(orientation="h",y=-0.25), height=320, margin=dict(t=20,b=70,l=40,r=10))
            st.plotly_chart(fig_aa, use_container_width=True, config={"displayModeBar":False})
            rows_html = "".join(f"<tr><td style='color:#b0d8f0;font-family:monospace;padding:2px 8px;'>{aa}</td><td style='color:#3a6080;padding:2px 8px;'>{aa_counts[aa]}</td><td style='color:#38bdf8;padding:2px 8px;'>{round(aa_counts[aa]/len(seq_c)*100,1)}%</td></tr>" for aa in sorted(aa_counts, key=lambda a: aa_counts[a], reverse=True))
            st.markdown(f"<table style='font-size:.75rem;border-collapse:collapse;'><thead><tr><th style='color:#1e4060;padding:2px 8px;'>AA</th><th style='color:#1e4060;padding:2px 8px;'>Count</th><th style='color:#1e4060;padding:2px 8px;'>%</th></tr></thead><tbody>{rows_html}</tbody></table>", unsafe_allow_html=True)

        elif _chem_mode == " Hydrophobicity Map":
            sh("", f"Kyte-Doolittle Hydrophobicity — {gene}")
            st.markdown("<div style='color:#3a6080;font-size:.8rem;margin-bottom:.4rem;'>Sliding window (w=9). Score >1.8 = likely transmembrane/buried hydrophobic core. Score <-1.6 = surface-exposed hydrophilic loop. GPCRs show 7 peaks above threshold.</div>", unsafe_allow_html=True)
            hydro_profile = kyte_doolittle(seq_c, window=9)
            step = max(1, len(hydro_profile)//800)
            hp_x = [h[0] for h in hydro_profile[::step]]
            hp_y = [h[1] for h in hydro_profile[::step]]
            fig_h = go.Figure()
            fig_h.add_trace(go.Scatter(x=hp_x, y=[max(0,v) for v in hp_y], mode="lines", fill="tozeroy", line=dict(color="#ff8c42",width=0), fillcolor="rgba(255,140,66,.2)", name="Hydrophobic", showlegend=False))
            fig_h.add_trace(go.Scatter(x=hp_x, y=[min(0,v) for v in hp_y], mode="lines", fill="tozeroy", line=dict(color="#4a90d9",width=0), fillcolor="rgba(74,144,217,.2)", name="Hydrophilic", showlegend=False))
            fig_h.add_trace(go.Scatter(x=hp_x, y=hp_y, mode="lines", line=dict(color="#38bdf8",width=1.2), name="KD score"))
            fig_h.add_hline(y=1.8, line=dict(color="#ff2d55",dash="dot",width=1), annotation_text="TM threshold 1.8", annotation_position="top right")
            fig_h.add_hline(y=0, line=dict(color="#1e4060",width=0.5))
            fig_h.update_layout(plot_bgcolor="#020617", paper_bgcolor="#020617", font_color="#b0d8f0", font_size=10, xaxis=dict(gridcolor="#0a1828", title=f"Position (1–{len(seq_c)})"), yaxis=dict(gridcolor="#0a1828", title="KD score"), height=340, margin=dict(t=20,b=40,l=40,r=10))
            st.plotly_chart(fig_h, use_container_width=True, config={"displayModeBar":False})
            tm_regions = []
            in_tm = False; tm_start = 0
            for pos, score in hydro_profile:
                if score > 1.8 and not in_tm: in_tm=True; tm_start=pos
                elif score <= 1.8 and in_tm:
                    in_tm=False
                    if pos-tm_start >= 17: tm_regions.append((tm_start, pos-1))
            if tm_regions:
                st.markdown(f"<div style='color:#ff8c42;font-size:.76rem;font-weight:700;margin:.5rem 0 .3rem;'>Predicted TM regions: {len(tm_regions)} segments</div>", unsafe_allow_html=True)
                for s, e in tm_regions:
                    st.markdown(f"<div style='color:#3a6080;font-size:.73rem;padding:2px 0;'>aa {s}–{e} ({e-s+1} residues)</div>", unsafe_allow_html=True)
            else:
                st.info("No predicted transmembrane segments found.")

        elif _chem_mode == " Phosphorylation Sites":
            sh("", f"Phosphorylation Chemistry — {gene}")
            phospho_feats = [f for f in pdata.get("features",[]) if f.get("type") in ("Modified residue","Glycosylation") and any(k in str(f).lower() for k in ["phospho","glyco"])]
            all_sty = [(i+1, seq_c[i]) for i in range(len(seq_c)) if seq_c[i] in "STY"]
            ser_p = [(p,aa) for p,aa in all_sty if aa=="S"]
            thr_p = [(p,aa) for p,aa in all_sty if aa=="T"]
            tyr_p = [(p,aa) for p,aa in all_sty if aa=="Y"]
            col_ph1, col_ph2 = st.columns([1.3, 0.8])
            with col_ph1:
                fig_p = go.Figure()
                fig_p.add_trace(go.Scatter(x=[p for p,_ in ser_p], y=["S"]*len(ser_p), mode="markers", marker=dict(color="#38bdf8",size=4), name="Ser"))
                fig_p.add_trace(go.Scatter(x=[p for p,_ in thr_p], y=["T"]*len(thr_p), mode="markers", marker=dict(color="#ffd60a",size=4,symbol="diamond"), name="Thr"))
                fig_p.add_trace(go.Scatter(x=[p for p,_ in tyr_p], y=["Y"]*len(tyr_p), mode="markers", marker=dict(color="#f97316",size=5,symbol="star"), name="Tyr"))
                for feat in phospho_feats:
                    loc = feat.get("location",{}); pos = int(loc.get("start",{}).get("value",0) or 0)
                    if pos: fig_p.add_vline(x=pos, line=dict(color="#22c55e",width=1,dash="dot"))
                fig_p.update_layout(plot_bgcolor="#020617", paper_bgcolor="#020617", font_color="#b0d8f0", font_size=10, xaxis=dict(gridcolor="#0a1828", title=f"Position (1–{len(seq_c)})"), yaxis=dict(gridcolor="#0a1828"), height=260, margin=dict(t=10,b=40,l=40,r=10), legend=dict(orientation="h",y=-0.3))
                st.plotly_chart(fig_p, use_container_width=True, config={"displayModeBar":False})
            with col_ph2:
                st.markdown(f"<div style='background:#050d24;border:1px solid #f9731622;border-radius:10px;padding:12px;'><div style='color:#f97316;font-weight:700;font-size:.8rem;margin-bottom:.5rem;'>Summary</div><div style='color:#3a6080;font-size:.75rem;line-height:1.8;'>Ser: <b style='color:#38bdf8;'>{len(ser_p)}</b><br>Thr: <b style='color:#ffd60a;'>{len(thr_p)}</b><br>Tyr: <b style='color:#f97316;'>{len(tyr_p)}</b><br>Total: <b style='color:#b0d8f0;'>{len(all_sty)}</b><br>UniProt annotated: <b style='color:#22c55e;'>{len(phospho_feats)}</b></div></div>", unsafe_allow_html=True)
                if phospho_feats:
                    st.markdown("<div style='color:#22c55e;font-size:.72rem;font-weight:700;margin-top:.5rem;'>Annotated sites:</div>", unsafe_allow_html=True)
                    for feat in phospho_feats[:10]:
                        loc = feat.get("location",{}); pos = int(loc.get("start",{}).get("value",0) or 0)
            ...
            try:
                ...
                st.warning(f"Backbone renderer: {str(_cbe)[:80]}")

    else:
        st.info("Search a protein in the sidebar to explore its chemical profile.")
