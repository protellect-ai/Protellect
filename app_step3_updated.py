...
# Payments are not yet wired to a real Stripe account. Until STRIPE_LINKS values
# are replaced with real payment links, upgrade buttons should say so honestly
# instead of linking to a dead placeholder URL.
_STRIPE_LIVE = {k: "placeholder" not in v for k, v in STRIPE_LINKS.items()}
...
def login_page():
    ...
    with col_m:
        ...
        with tab_up:
            ...
            st.markdown(
                "<div style='background:#1a1400;border:1px solid #8a704044;border-radius:8px;padding:.5rem .7rem;margin-bottom:.7rem;color:#d4a03a;font-size:.76rem;'>"
                " Beta: accounts are stored for this session only and are not yet saved permanently. "
                "Your account and analysis history will not persist after you close or refresh the app."
        ...
        with tab_plans:
            st.markdown(
                ...
                + (f"<a href='{STRIPE_LINKS['pro']}' target='_blank' style='display:inline-block;margin-top:.5rem;background:#38bdf8;color:#000;font-weight:700;padding:4px 18px;border-radius:8px;font-size:.82rem;text-decoration:none;'>Upgrade to Pro</a>"
                   if _STRIPE_LIVE["pro"] else
                   "<span style='display:inline-block;margin-top:.5rem;background:#1a2530;color:#5a8090;font-weight:700;padding:4px 18px;border-radius:8px;font-size:.82rem;'>Payments coming soon</span>")
                + "</div>"
                ...
                + (f"<a href='{STRIPE_LINKS['enterprise']}' target='_blank' style='display:inline-block;margin-top:.5rem;background:#a855f7;color:#fff;font-weight:700;padding:4px 18px;border-radius:8px;font-size:.82rem;text-decoration:none;'>Upgrade to Enterprise</a>"
                   if _STRIPE_LIVE["enterprise"] else
                   "<span style='display:inline-block;margin-top:.5rem;background:#1a2530;color:#5a8090;font-weight:700;padding:4px 18px;border-radius:8px;font-size:.82rem;'>Payments coming soon</span>")
                + "</div>",
                unsafe_allow_html=True,
            )
...
def verify_ai_claims(
    result: dict, gene: str, pdata: dict, cv: dict, gi: dict,
    gnomad: dict, string_data: list, drugs: list, trials: list,
    abstracts: list,
) -> dict:
    """
    Post-hoc verification pass for ai_synthesize() output.

    This is a heuristic, rule-based checker — NOT a guarantee of correctness.
    It catches a specific, real class of error: numbers, PMIDs, and gene
    references that appear in the LLM's free-text output but do not trace
    back to anything in the source data that was actually passed to it. It
    will NOT catch subtler forms of hallucination (e.g. a correct-looking
    number attached to the wrong claim, or a plausible but false causal
    story). Treat a clean flag_rate as "no crude fabrication detected," not
    as "this output is correct."

    Returns a dict: {
        "checked_fields": [...],
        "flagged_claims": [{"field": ..., "claim": ..., "reason": ...}, ...],
        "verified_numeric_count": int,
        "flagged_numeric_count": int,
        "flag_rate": float,   # flagged / (flagged + verified), 0.0 if nothing to check
    }
    """
    import re as _re2

    # ── Build the set of facts that are actually legitimate to reference ──
    known_pmids = set()
    for p in abstracts or []:
        pmid = str(p.get("pmid", "") or p.get("pubmed_id", "")).strip()
        if pmid:
    ...
    def _add_num(x):
        try:
            f = float(x)
            known_numbers.add(round(f, 2))
        except (TypeError, ValueError):
            pass

    _add_num(gnomad.get("pLI"))
    _add_num(gnomad.get("oe_lof"))
    _add_num(gnomad.get("oe_mis"))
    _add_num(gi.get("n_pathogenic", 0))
    _add_num(gi.get("n_vus", 0))
    _add_num(gi.get("n_benign", 0))
    _add_num(cv.get("summary", {}).get("total", 0))
    if gi.get("density") is not None:
        _add_num(round(gi.get("density", 0) * 100, 2))
    _add_num(len(string_data or []))
    _add_num(len(drugs or []))
    _add_num(len(trials or []))
    _add_num(len(abstracts or []))
    for i in string_data or []:
        _add_num(i.get("score"))

    known_gene_tokens = {gene.upper()} if gene else set()
    for i in string_data or []:
        partner = str(i.get("partner", "")).upper()
        if partner:
            known_gene_tokens.add(partner)

    # ── Fields to scan (the free-text fields the model actually writes) ──
    text_fields = [
        "one_line_verdict", "executive_summary", "organism_note",
        "interaction_insights", "population_genetics_interpretation",
        "drug_opportunity", "clinical_translation", "assay_interpretation",
    ]
    ...
    pmid_pattern = _re2.compile(r'PMID:?\s*(\d{4,9})', _re2.IGNORECASE)
    # Only flag numbers with a decimal point or 3+ digits — short integers
    # are almost always list positions/counts, not factual claims, and
    # flagging them produces noise that trains reviewers to ignore flags.
    number_pattern = _re2.compile(r'\b\d+\.\d+\b|\b\d{3,}\b')

    def _scan_text(field_name: str, text: str):
        nonlocal verified_numeric, flagged_numeric
        if not text or not isinstance(text, str):
            return
        for m in pmid_pattern.finditer(text):
            pmid = m.group(1)
            if known_pmids and pmid not in known_pmids:
                flagged.append({
                    "field": field_name, "claim": f"PMID:{pmid}",
                    "reason": "Cited PMID does not match any abstract passed into the prompt.",
                })
            elif not known_pmids:
                flagged.append({
                    "field": field_name, "claim": f"PMID:{pmid}",
                    "reason": "PMID cited but no abstracts were provided as source data at all.",
                })
        for m in number_pattern.finditer(text):
            try:
                ...
                continue
            if val in known_numbers or any(abs(val - k) < 0.05 for k in known_numbers):
                verified_numeric += 1
            else:
                flagged_numeric += 1
                flagged.append({
                    "field": field_name, "claim": m.group(),
                    "reason": "Numeric value does not match any figure in the source data provided.",
                })

    for f in text_fields:
        _scan_text(f, result.get(f, ""))

    for ch in result.get("cure_hypotheses", []) or []:
        _scan_text("cure_hypotheses.citation_basis", ch.get("citation_basis", ""))
    for lp in result.get("literature_precedents", []) or []:
        _scan_text("literature_precedents.source", lp.get("source", ""))
    for ed in result.get("experiments_done", []) or []:
        pmid = str(ed.get("pmid", "")).strip()
        if pmid and pmid.lower() not in ("", "n/a", "not available", "none"):
            if known_pmids and pmid not in known_pmids:
                flagged.append({
                    "field": "experiments_done.pmid", "claim": f"PMID:{pmid}",
                    "reason": "Cited PMID does not match any abstract passed into the prompt.",
                })

    total_checked = verified_numeric + flagged_numeric
    flag_rate = round(flagged_numeric / total_checked, 3) if total_checked else 0.0

    return {
        "checked_fields": text_fields,
        "flagged_claims": flagged,
        "verified_numeric_count": verified_numeric,
        "flagged_numeric_count": flagged_numeric,
        "flag_rate": flag_rate,
    }
...
=== YOUR TASK ===
Based ONLY on the data explicitly provided above, produce a JSON response. If a field cannot be supported by the data above, state "not available in current data" for that field rather than filling it in from general knowledge:
    ...
    try:
        ...
        response = requests.post(
            ...
            json={
                ...
                # NOTE: web_search tool intentionally removed. Giving the model live
                # internet access here directly contradicts the "use ONLY the data
                # provided above" instruction in the prompt text — a text instruction
                # cannot override a tool the model is actually able to call. Grounding
                # requires both the instruction AND the absence of an escape route.
            },
            timeout=60
        )
        ...
        if json_match:
            try:
                ...
                parsed["_verification"] = verify_ai_claims(
                    parsed, gene, pdata, cv, gi, gnomad, string_data, drugs, trials, abstracts
                )
                return parsed
            except Exception:
                pass
    ...
    if not check_search_limit():
        st.markdown(
            ...
            + (f"<a href='{STRIPE_LINKS['pro']}' target='_blank' style='background:#38bdf8;color:#000;font-weight:700;"
               ...
               "Upgrade to Pro — $49/month</a>"
               if _STRIPE_LIVE["pro"] else
               "<span style='background:#1a2530;color:#5a8090;font-weight:700;"
               "padding:4px 18px;border-radius:8px;font-size:.82rem;display:inline-block;margin-top:.3rem;'>"
               "Pro plan payments coming soon</span>")
            + "</div>",
            unsafe_allow_html=True,
        )
        st.stop()
...
with tab5:
    ...
    if not ai:
        ...
        if verdict:
            ...
            _verif = ai.get("_verification") or {}
            _flagged = _verif.get("flagged_claims", [])
            if _flagged:
                st.markdown(
                    "<div style='background:#1a0a00;border:1px solid #ff8c4266;border-radius:10px;"
                    "padding:.7rem 1rem;margin-bottom:.8rem;'>"
                    f"<div style='color:#ff8c42;font-weight:700;font-size:.82rem;margin-bottom:4px;'>"
                    f" {len(_flagged)} unverified claim(s) detected in this AI report "
                    f"(numeric flag rate: {_verif.get('flag_rate',0)*100:.0f}%)</div>"
                    "<div style='color:#8a6a4a;font-size:.76rem;line-height:1.6;'>"
                    "These values or citations appeared in the AI's text but could not be matched to the "
                    "source data provided to it. This does not necessarily mean they are wrong — but they "
                    "have not been confirmed against your data and should be checked before being relied on."
                    "</div></div>",
                    unsafe_allow_html=True,
                )
                with st.expander(f"Show {len(_flagged)} flagged claim(s)"):
                    for fc in _flagged:
                        st.markdown(
                            f"<div style='color:#8a6a4a;font-size:.78rem;padding:3px 0;border-bottom:1px solid #2a1a0a;'>"
                            f"<b>{fc['field']}</b>: \"{fc['claim']}\" — {fc['reason']}</div>",
                            unsafe_allow_html=True,
                        )
            elif _verif:
                st.markdown(
                    "<div style='color:#2a5060;font-size:.72rem;margin-bottom:.8rem;'>"
                    f" No unverified numeric claims or citations detected "
                    f"({_verif.get('verified_numeric_count',0)} checked against source data).</div>",
                    unsafe_allow_html=True,
                )
